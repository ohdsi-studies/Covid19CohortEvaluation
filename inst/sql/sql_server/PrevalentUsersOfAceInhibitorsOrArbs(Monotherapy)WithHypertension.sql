...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (991382,932745,956874,970250,904542,1309799,942350)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (991382,932745,956874,970250,904542,1309799,942350)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (1340128,1341927,1363749,1308216,1367500,1373225,1334456,40235485,1335471,1351557,1346686,1347384,1310756,40226742,1331235,1317640,1342439,1308842)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (1340128,1341927,1363749,1308216,1367500,1373225,1334456,40235485,1335471,1351557,1346686,1347384,1310756,40226742,1331235,1317640,1342439,1308842)
  and c.invalid_reason is null
...
(
  select E.person_id, E.start_date, E.end_date,
         row_number() OVER (PARTITION BY E.person_id ORDER BY E.sort_date DESC) ordinal,
...
JOIN #Codesets codesets on ((de.drug_concept_id = codesets.concept_id and codesets.codeset_id = 19))
) C

WHERE (C.drug_exposure_start_date >= DATEFROMPARTS(2019, 11, 01) and C.drug_exposure_start_date <= DATEFROMPARTS(2020, 01, 31))
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= DATEADD(day,-180,P.START_DATE) AND A.END_DATE >= DATEADD(day,0,P.START_DATE) AND A.END_DATE <= P.OP_END_DATE
...
FROM (
	...
	JOIN #Codesets cs on cs.codeset_id = 19 AND de.drug_concept_id = cs.concept_id
	...
	FROM @cdm_database_schema.DRUG_EXPOSURE de
	JOIN ctePersons p on de.person_id = p.person_id
	JOIN #Codesets cs on cs.codeset_id = 19 AND de.drug_source_concept_id = cs.concept_id
) E
;
